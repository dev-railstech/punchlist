...
function NewPunchList(project_id, project_name, client_name) {
	...
	var pl_title = Ti.UI.createTextField({
		top:94,
		height:25,
		left:80,
		width:230,
	...
	var create_btn = Ti.UI.createButton({
		top:220,
		right:40,
		left:40,
		title:'Create'
	...
	create_btn.addEventListener('click',function(){
		...
		var pl_ttl = pl_title.value;
		var visit_date = pl_date.value;
		if(pl_ttl.length < 1 || visit_date.length < 1 || selected_to == 0){
			alert("Please fill in all fields");
			loader.hide();
			return;
		}
		var url = 'https://dev.masterlibrary.com/Service/Mobile.svc/PunchListAdd';
		var Data = {"ProjectId": project_id,"PLVisitDate" : visit_date , "PLTitle":pl_ttl , "EnteredBy" : Titanium.App.Properties.getString('userId'),"ActionBy":selected_to,"IssueId":selected_issue};
		...
		xhr.onload = function(){	
			...
			if(this.status == 200){
				//setTimeout(function() {
					...
					var punchListDetail = PunchListDetail(project_id, project_name, client_name,this.responseText,'New punch list',pl_ttl);
					punchListDetail.left= width;
					punchListDetail.open(slide_left);
					showMessageTimeout("Punch list Successfully Created");
					self.close();
					self = null;
					win1.close();
					win1 = null;
	...
	function selectDateFromCal(){
		if(datePicker==null){
			...
			var okButtonClick = okButton.addEventListener('click', function(e) {
				win.hide();
